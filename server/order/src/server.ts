...
import { orderKafkaService } from '@/services/kafkaService';
...
class OrderServer {
    ...
    private async checkKafkaHealth(): Promise<boolean> {
        try {
            ...
            return await orderKafkaService.isHealthy();
        } catch (error) {
            logger.debug('Kafka health check failed', error);
            return false;
        }
    }
