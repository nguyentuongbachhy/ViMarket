...
type ServiceProxy struct {
	...
	notificationsProxy *httputil.ReverseProxy
...
func NewServiceProxy(cfg *config.Config, logger *logrus.Logger) (*ServiceProxy, error) {
	...
	notificationURL, err := url.Parse(cfg.Services.Notifications)
	if err != nil {
		return nil, err
	}
	...
	notificationsProxy := httputil.NewSingleHostReverseProxy(notificationURL)
	...
	configureProxy(notificationsProxy, "notifications", logger)
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasPrefix(path, "/cart") {
			...
			p.logger.WithFields(logrus.Fields{
				...
				"path":    r.URL.Path,
				"method":  r.Method,
			}).Info("Routing to Notifications service")

			p.notificationsProxy.ServeHTTP(w, r)
