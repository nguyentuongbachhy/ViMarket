...
import type { CartExpirationEvent, CartItemAddedEvent } from '@/types';
...
export class CartKafkaService {
    ...
    async sendCartItemAddedEvent(event: CartItemAddedEvent): Promise<void> {
        if (!this.isConnected) {
            logger.warn('Kafka producer not connected, skipping cart item added event');
            return;
        }

        try {
            await this.producer.send({
                topic: 'cart.item.added', // Thêm topic mới
                messages: [
                    {
                        key: `${event.userId}:${event.productId}`,
                        value: JSON.stringify(event),
                        timestamp: Date.now().toString(),
                        headers: {
