...
import { CartItem, CartItemAddedEvent, CartItemValidation, CartItemWithProduct, CartValidationResult, CartWithProducts, GuestCartItem, ProductSummary } from '@/types';
import { Logger } from '@/utils/logger';
import { v4 } from 'uuid';
import { cartKafkaService } from './kafkaService';
...
export class CartService {
    ...
    async addToCart(userId: string, productId: string, quantity: number): Promise<CartWithProducts> {
        try {
            ...
            try {
                const addedItem = cartWithProducts.items.find(item => item.productId === productId);
                if (addedItem) {
                    logger.info('🔄 Preparing to send cart item added event', {
                        userId,
                        productId,
                        productName: addedItem.product.name,
                        cartKafkaServiceConnected: cartKafkaService.isHealthy(),
                    });
                    ...
                    const event: CartItemAddedEvent = {
                        ...
                        productId,
                        productName: addedItem.product.name,
                        productPrice: addedItem.product.price,
                        productImage: addedItem.product.images?.[0]?.url,
                        quantity,
                        totalCartItems: cartWithProducts.totalItems,
                        totalCartValue: cartWithProducts.pricing.total,
                        timestamp: new Date().toISOString(),
                    };

                    await cartKafkaService.sendCartItemAddedEvent(event);
                    logger.info('✅ Cart item added event sent successfully', {
                ...
                logger.error('❌ Failed to send cart item added event', {
                    error: kafkaError,
                    userId,
                    productId,
                    errorMessage: kafkaError instanceof Error ? kafkaError.message : 'Unknown error',
                });
            }
