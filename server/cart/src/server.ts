...
import { cartKafkaService } from '@/services/kafkaService';
...
class CartServer {
  ...
  private async connectToKafka(): Promise<void> {
    try {
      ...
      await cartKafkaService.connect();
      logger.info('✅ Kafka connected successfully');
    } catch (error) {
      logger.warn('⚠️ Failed to connect to Kafka (optional service)', error);
      // Don't throw error since Kafka is optional
    }
  }
  ...
  private async checkKafkaHealth(): Promise<boolean> {
    try {
      return cartKafkaService.isHealthy();
    } catch (error) {
      return false;
    }
  }
